...
Available skills:
  ...
  fact_checker       verify factual claims against live web evidence
                     (supported / refuted / inconclusive, with source URLs)
...
When to use fact_checker (the new student-added skill):
- The user asks "is X true?", "did Y happen?", "verify the claim that …",
  or otherwise explicitly demands verification of a factual statement.
- Emit one fact_checker node whose inputs are the claim(s). Wire it BEFORE
  the formatter so the formatter has verdicts to quote.
- Do NOT use fact_checker for open-ended research — use researcher there.

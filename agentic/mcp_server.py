...
# ── S7: vector memory tools ──────────────────────────────────────────────────
# The MCP server runs as a stdio subprocess but writes to the *same*
# state/memory.json + state/index.faiss as the agent process. Lazy-imported
# so circular dependencies stay clean.

def _chunk_text(text: str, *, size: int = 400, overlap: int = 80) -> list[str]:
    """Slide a fixed-size word window with overlap. Deliberately naive — S8/S9
    introduce semantic chunking. Bigger window = more meaning per chunk but
    diluted relevance; smaller = finer-grained but more chunks to retrieve."""
    words = text.split()
    if not words:
        return []
...
def _get_memory():
    """Local Memory instance for the MCP subprocess. Reads/writes the same
    state files as the agent. Lazy to avoid cold-start cost when the agent
    only uses non-memory tools."""
    from gateway_client import Gateway
    from memory import Memory
    state_path = Path(__file__).parent / "state" / "memory.json"
    return Memory(Gateway(), path=str(state_path))


@mcp.tool()
def index_document(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    """Read a file from the sandbox, chunk it into overlapping word windows,
    embed each chunk via the gateway, and store as a searchable `fact` in Memory.

    Use this when content must remain retrievable across later turns or runs
    (the *indexing* step before later vector queries). For one-shot inspection
    of a known file in this turn, prefer `read_file`.

    Example: index_document("papers/attention.md")"""
    # Be lenient: the user may pass "sandbox/papers/x.md" thinking the path
    # is repo-relative, but our `_safe` already roots in ./sandbox/. Strip
    # one redundant "sandbox/" prefix if present.
    if path.startswith("sandbox/") or path.startswith("./sandbox/"):
        path = path.split("sandbox/", 1)[1]
    p = _safe(path)
    if not p.exists():
    ...
    if not chunks:
        return {"path": path, "chunks_indexed": 0, "warning": "empty content"}
    mem = _get_memory()
    run_id = f"index-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    for i, chunk in enumerate(chunks):
        preview = chunk[:120].replace("\n", " ")
        descriptor = f"[{path} chunk {i+1}/{len(chunks)}] {preview}"
        mem.add_fact(
            descriptor=descriptor,
            value={"chunk": chunk, "chunk_index": i,
                   "total_chunks": len(chunks), "source": path},
            source=f"sandbox:{path}", run_id=run_id,
        )
...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    """Vector-search indexed chunks. Returns up to k ranked chunks with
    provenance (source file, chunk index). Use this rather than re-fetching
    or re-reading sources whenever Memory already contains indexed chunks
    for the topic — that's the whole point of indexing.

    Example: search_knowledge("authentication flow", 5)"""
    mem = _get_memory()
    items = mem.read(query, kinds=["fact"], top_k=k)
    return [
        {
            "id": it.id,
            "descriptor": it.descriptor,
            "source": it.source,
            "chunk": it.value.get("chunk", "")[:1200],
            "chunk_index": it.value.get("chunk_index"),
            "total_chunks": it.value.get("total_chunks"),
        }
        for it in items
    ]

...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer_use":
        # Session 10. Same dispatch shape as browser/sandbox_executor: the
        # skill owns its own four-layer cascade over cua-driver and never
        # touches the LLM tool/text channel. Bypass render_prompt and
        # gateway-chat entirely.
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
            skill="computer_use",
            inputs=node_dict.get("inputs") or [],
